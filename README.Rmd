...
datasets, taxa and name usages, navigate the taxonomic tree, and run the
...
flavours (a base release and an extended release `XR`). Every function takes a
`dataset =` argument; helpful aliases resolve to the latest of each:
...
`CLB_BASE_URL` environment variable, e.g. to the development server or to a
locally running [dockerized matching container](https://www.catalogueoflife.org/tools/matching):
...
[dockerized matching containers](https://www.catalogueoflife.org/tools/matching)
that bundle a release and expose a matching service locally:
